...
{
#ifdef DEBUGGING
    printf("PyLogger (name %s) %s created\n", _name.c_str(), getFullName().c_str());
#endif
...

{
  ...
  if (lvl == TRACE) {
      ...
      cerr << "unknonw lvl" << lvl << endl;
  }
  std::cout << "Py::log<" << level << ", " << pylvl << "> called "
            << msg << std::endl;

  if (level < pylvl) {
      cout << "skip log message (loglevel condition)" << endl;
      return;
  }
  if (pyLogger == object()) return;
  // cout << "py::log tmp" << endl;
...

{
  ...
  printf("setPythonLevel: level of logger %s is now %d, setflag = %d\n",
          getFullName().c_str(), pyLevel, setFlag);
#endif
...
{
  if (pyLogger == object()) {
      // auto name = extract<std::string>(_pyLogger.attr("name"));
      string name = extract<string>(_pyLogger.attr("name"));
      std::cout << "cpp: python logger set to " << name << std::endl;
      pyLogger = _pyLogger;
      if (pyLogger == object()) {
  ...
  cout << "setting level of logger '" << name << "' to " << level << endl;
...
void loggerUpdate(object pythonLogger) {
  ...
  std::cout << "name of logger " << name << std::endl;
